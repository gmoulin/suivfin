...
class currency extends common {
	 ...
	 * errors are returned with form inputs ids as (id, text, currency)
	 *
	 * @return array[]
	 */
		...
		if( empty($errors) ){
			...
			if( $action == 'update' ){
				if( is_null($id) || $id === false ){
					...
					if( $id === false ){
						$errors[] = array('name', 'Identifiant de la monnaie incorrect.', 'error');
					} else {
						//check if id exists in DB
						if( self::existsById($id) ){
							$formData['id'] = $id;
						} else {
							$errors[] = array('name', 'Identifiant de la monnaie inconnu.', 'error');
						}
					}
				}
			}
			...
			if( $action == 'update' || $action == 'add' ){
				...
				//symbol
				if( is_null($symbol) || $symbol === false ){
					$errors[] = array('symbol', 'Symbole incorrect.', 'error');
				} elseif( empty($symbol) ){
					$errors[] = array('symbol', 'Le symbole est requis.', 'required');
				} else {
					$formData['name'] = trim($name);
				}
				...
				if( empty($errors) ){
					...
					if( $check ){
						if( $action == 'add' || ($action == 'update' && $formData['id'] != $check) ){
							$errors[] = array('name', 'Cette monnaie est déjà présente.', 'error');
						}
					}
				}
			}
	...
	public function delete(){
		try {
			if( parent::delete() === true ){
				$q = $this->_db->prepare('DELETE FROM balance where currencyFK = :id');
				$q->excute(array(
					':id' => $this->_data['id'],
				));
			}
