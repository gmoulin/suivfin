...
class location extends common {
	 ...
	 * errors are returned with form inputs ids as (id, text, location)
	 *
	 * @return array[]
	 */
		...
		if( empty($errors) ){
			...
			if( $action == 'update' ){
				if( is_null($id) || $id === false ){
					...
					if( $id === false ){
						$errors[] = array('name', 'Identifiant de la localisation incorrect.', 'error');
					} else {
						//check if id exists in DB
						if( self::existsById($id) ){
							$formData['id'] = $id;
						} else {
							$errors[] = array('name', 'Identifiant de la localisation inconnu.', 'error');
						}
					}
				}
			}
				...
				if( empty($errors) ){
					...
					if( $check ){
						if( $action == 'add' || ($action == 'update' && $formData['id'] != $check) ){
							$errors[] = array('name', 'Cette localisation est déjà présente.', 'error');
						}
					}
				}
			}
