<?php
/**
 * Class for paiement recipient management
 *
 ...
 * @category Recipients
 */
class recipient extends common {
	 ...
	 * errors are returned with form inputs ids as (id, text, recipient)
	 *
	 * @return array[]
	 */
		...
		if( empty($errors) ){
			...
			if( $action == 'update' ){
				if( is_null($id) || $id === false ){
					...
					if( $id === false ){
						$errors[] = array('name', 'Identifiant du bénificiaire incorrect.', 'error');
					} else {
						//check if id exists in DB
						if( self::existsById($id) ){
							$formData['id'] = $id;
						} else {
							$errors[] = array('name', 'Identifiant du bénificiaire inconnu.', 'error');
						}
					}
				}
			}
				...
				if( empty($errors) ){
					...
					if( $check ){
						if( $action == 'add' || ($action == 'update' && $formData['id'] != $check) ){
							$errors[] = array('name', 'Ce bénificiaire est déjà présent.', 'error');
						}
					}
				}
			}
