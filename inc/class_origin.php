...
class origin extends common {
	 ...
	 * errors are returned with form inputs ids as (id, text, origin)
	 *
	 * @return array[]
	 */
		...
		if( empty($errors) ){
			...
			if( $action == 'update' ){
				if( is_null($id) || $id === false ){
					...
					if( $id === false ){
						$errors[] = array('name', 'Identifiant de l\'origine incorrect.', 'error');
					} else {
						//check if id exists in DB
						if( self::existsById($id) ){
							$formData['id'] = $id;
						} else {
							$errors[] = array('name', 'Identifiant de l\'origine inconnu.', 'error');
						}
					}
				}
			}
				...
				if( empty($errors) ){
					...
					if( $check ){
						if( $action == 'add' || ($action == 'update' && $formData['id'] != $check) ){
							$errors[] = array('name', 'Cette origine est déjà présente.', 'error');
						}
					}
				}
			}
